...
from FocusOn.spotify import spotifyAPI
...
def get_playlists_tracks(id_playlist, token):
    data = spotifyAPI.get_tracks_from_playlist_call(id_playlist, token)
    d = data
    result = {'list': []}
    for item in d['items']:
        track = {
...
def get_info_tracks(data, token):
    ...
    result = spotifyAPI.get_track_info_call(input_ids, token)
    result_file = {}
    d = json.loads(result)
...
def get_info_track(track_id, token):
    result = spotifyAPI.get_track_info_call(track_id, token)
    result_file = {}
    d = json.loads(result)
    for item in d['audio_features']:
        res = {
...
def pauseSong(token):
    spotifyAPI.pause_song_call(token)

def playSong(token, device_id):
    spotifyAPI.play_song_call(token, device_id)

def nextNong(token, device_id):
    spotifyAPI.next_song_call(token, device_id)

if __name__ == '__main__':

    print(spotifyAPI.getAuthToken())
    #token = authorize(auth_token)
